...
import static com.utils.gui_utils.charts.utils.JFXUtil.getXShift;
import static com.utils.gui_utils.charts.utils.JFXUtil.getYShift;
...
public class XYChartInfo<
	...
	Point2D getDataCoordinates(
		...
		final double xStart = getXShift(xAxis, referenceNode);
		final double yStart = getYShift(yAxis, referenceNode);
	...
	private Rectangle2D getComponentArea(
			final Region childRegion) {

		final double xStart = getXShift(childRegion, referenceNode);
		final double yStart = getYShift(childRegion, referenceNode);
