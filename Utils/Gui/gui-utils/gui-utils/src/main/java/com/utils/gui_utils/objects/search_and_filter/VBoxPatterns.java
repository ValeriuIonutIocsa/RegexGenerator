...
public class VBoxPatterns extends CustomControlAbstr<VBox> {
	...
	protected VBox createRoot() {
		...
		vBoxRoot.getStylesheets().add("com/utils/gui_utils/objects/search_and_filter/patterns.css");

		final GridPane gridPanePatternTable = createGridPanePatternTable();
		final ScrollPane scrollPaneSearchPatterns =
				LayoutControlsFactory.createScrollPane(gridPanePatternTable);
	...
	private static StackPane createLabelOperation(
		...
		final Label labelOperation =
				BasicControlsFactory.createLabel(filterType.name(), "label-operation");
		final StackPane stackPaneLabelOperation =
				LayoutControlsFactory.createStackPane(Pos.CENTER, labelOperation);
