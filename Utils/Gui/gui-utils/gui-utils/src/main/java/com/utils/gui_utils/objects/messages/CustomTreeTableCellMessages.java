...
class CustomTreeTableCellMessages extends CustomTreeTableCell<TableRowDataMessage, Object> {
	...
	public void startEdit() {
		...
		if (notCategory) {
			...
			final HBox hBoxTextArea = LayoutControlsFactory.createHBox();
	...
	public void setText(
		...
		if (category) {
			...
			if (messageType == MessageType.APPROVAL || messageType == MessageType.INFO) {
				createUserActionRequiredComponent(stackPane, tableRowDataMessage, item, null);

			} else if (messageType == MessageType.WARNING) {
				createUserActionRequiredComponent(stackPane, tableRowDataMessage, item,
	...
	private void createUserActionRequiredComponent(
			final StackPane stackPane,
			final TableRowDataMessage tableRowDataMessage,
			final Object item,
			final String userActionRequiredText) {

		final HBox hBoxUserActionRequired = LayoutControlsFactory.createHBox();

		final String messageCategory = item.toString();
		final Label label = BasicControlsFactory.createLabel(messageCategory, "boldFontSize9");
		final Rectangle statusRectangle = createStatusRectangle(tableRowDataMessage);
		label.setGraphic(statusRectangle);
		GuiUtils.addToHBox(hBoxUserActionRequired, label,
				Pos.CENTER_LEFT, Priority.NEVER, 0, 0, 0, 0);

		final Image imageLongArrow = ImagesGuiUtils.createImageLongArrow();
		final ImageView imageViewArrow = new ImageView(imageLongArrow);
		...
		final Label labelUserActionRequired =
				BasicControlsFactory.createLabel(userActionRequiredText, "boldFontSize9");
