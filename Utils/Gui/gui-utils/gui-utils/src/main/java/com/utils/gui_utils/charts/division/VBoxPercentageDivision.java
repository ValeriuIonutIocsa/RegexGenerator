...
public class VBoxPercentageDivision extends CustomControlAbstr<VBox> {
	...
	private GridPane createGridPaneColors(
		...
		final GridPane gridPaneColors = LayoutControlsFactory.createGridPane();
		final int elementCount = elements.size();
		for (int i = 0; i < elementCount; i++) {
			...
			final ColumnConstraints percentageConstraints =
					LayoutControlsFactory.createPercentageConstraints(percentage);
	...
	private HBox createHBoxDescription(
		...
		final HBox hBoxDescription = LayoutControlsFactory.createHBox();
		...
		for (final PercentageDivisionElement element : elements) {
			...
			final Label labelDescription = BasicControlsFactory.createLabel(text, "fontSize10");
